...
Query results are written to **stdout** by default (JSONL format). Use `output: VAR_NAME` to capture small results into an environment variable. For large results, use `streaming: true` with `output_file`. When `output_file` references `${context.paths.artifacts_dir}`, artifact storage is auto-enabled and the file appears as a run artifact.
:::
