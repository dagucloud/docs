...
s3:
  ...
  secret_access_key: ${env.GCS_HMAC_SECRET}
...
Scoped references in S3 config fields (`access_key_id`, `secret_access_key`, `endpoint`, `bucket`, etc.) expand DAG-scoped values. OS environment variables like `$HOME` are **not** expanded directly. To use an OS value, import it in the `env:` block:
