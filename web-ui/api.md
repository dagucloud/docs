  ...
  "dag": {
    ...
    "steps": [
      ...
      },
      {
        ...
        "command": "python transform.py --input=${env.EXTRACTED_FILE}",
      ...
      },
      {
        ...
        "params": "{\"file\": \"${env.TRANSFORMED_FILE}\", \"table\": \"fact_sales\"}",
...
Retrieves the collected outputs from a completed DAG run. This endpoint reads the run's `outputs.json`, which is populated by string-form `output: NAME`, `stdout.outputs`, and `action: outputs.write`.
...
- `outputs`: Key-value pairs of collected outputs
  - String-form `output: NAME`, `stdout.outputs`, and `outputs.write` values appear here
  - String-form `output: NAME` keys are converted from `SCREAMING_SNAKE_CASE` to `camelCase`; `stdout.outputs` and `outputs.write` keys are preserved
...
- Object-form `output: {...}` does not participate in run-level outputs unless values are republished through `stdout.outputs` or `outputs.write`
...
```json
{
  ...
  "spec": "group: ETL\nschedule:\n  - \"0 2 * * *\"\n  - \"0 14 * * *\"\ndescription: Daily data processing pipeline for warehouse ETL\nenv:\n  DATA_SOURCE: postgres://prod-db:5432/analytics\nlog_dir: /var/log/dagu/pipelines\nhist_retention_days: 30\nmax_active_steps: 5\nparams:\n  - name: date\n    type: string\n    required: true\n  - name: environment\n    type: string\n    default: dev\n    enum: [dev, staging, prod]\n  - name: batch_size\n    type: integer\n    default: 1000\n    minimum: 1\n    maximum: 10000\nlabels: [production, etl, daily]\npreconditions:\n  - eval: \"$(date +%u)\"\n    expected: \"re:[1-5]\"\nsteps:\n  - id: extract\n    description: Extract data from source database\n    working_dir: /app/etl\n    run: python extract.py --date \"${params.date}\"\n    stdout: /logs/extract.out\n    stderr: /logs/extract.err\n\n  - id: transform\n    description: Apply transformations to extracted data\n    run: python transform.py\n    depends: [extract]\n\n  - id: load\n    run: warehouse-loader --batch-size \"${params.batch_size}\"\n    depends: [transform]\nhandler_on:\n  success:\n    run: notify.sh \"Pipeline completed successfully\"\n  failure:\n    run: alert.sh \"Pipeline failed\" high\n  exit:\n    run: cleanup_temp_files.sh",
  "errors": []
}
```
...
Deleting a workspace removes the workspace record only. It does not rewrite DAG files, historical DAG runs, users, or API keys.
