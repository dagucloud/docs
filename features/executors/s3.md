...
`${VAR}` references in S3 config fields (`accessKeyId`, `secretAccessKey`, `endpoint`, `bucket`, etc.) expand only DAG-scoped variables (`env:`, `params:`, `secrets:`, step outputs). OS environment variables like `$HOME` are **not** expanded. To use an OS value, import it in the `env:` block:
