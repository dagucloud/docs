...
- [Action schema v2](https://docs.dagu.sh/writing-workflows/yaml-specification) adds the new `run`, `action`, `with`, and `outputs` style for clearer workflow definitions while keeping existing workflows supported. See also [workflow outputs](https://docs.dagu.sh/writing-workflows/outputs).
- [Remote Dagu Actions](https://docs.dagu.sh/dagu-actions/execution-model) and [Official Dagu Actions](https://docs.dagu.sh/dagu-actions/official) make reusable workflow packages possible. Actions can be referenced by version, resolve their own workspace, expose typed outputs, and run as sub-DAGs. DuckDB is provided as the official [duckdb@v1 Dagu Action](https://docs.dagu.sh/dagu-actions/official/duckdb), not as a built-in action.
